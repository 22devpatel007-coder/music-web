    ...
    const fetchSongs = async () => {
      try {
        ...
        const ordered = playlist.songIds
          .map(sid => res.data.find(s => s.id === sid))
          .filter(Boolean);
        setSongs(ordered);
      } catch (err) {
        console.error('[PlaylistDetail]', err.message);
      }
      setLoading(false);
    };
    fetchSongs();
          ...
          <div style={styles.coverWrap}>
            {playlist.coverUrl
              ? <img src={playlist.coverUrl} alt={playlist.name} style={styles.cover}
                  onError={e => { e.target.src = 'https://placehold.co/160x160/1a1a1a/555?text=♪'; }} />
          ...
          <div style={styles.meta}>
            ...
            {songs.length > 0 && (
              <div style={styles.controls}>
                ...
                <button style={styles.shuffleBtn} onClick={handleShufflePlay}>⇌ Shuffle</button>
              </div>
            )}
          </div>
        </div>
...
const styles = {
  ...
  header:          { display: 'flex', gap: 32, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 28 },
  ...
  saveBtn:         { background: '#22c55e', color: '#000', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  cancelBtn:       { background: '#2d2d2d', color: '#9ca3af', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, cursor: 'pointer', fontFamily: 'inherit' },
  controls:        { display: 'flex', gap: 10, marginTop: 8 },
  playBtn:         { background: '#22c55e', color: '#000', border: 'none', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 700, cursor: 'pointer', fontFamily: 'inherit' },
  shuffleBtn:      { background: '#1a1a1a', color: '#9ca3af', border: '1px solid #2d2d2d', borderRadius: 8, padding: '10px 24px', fontSize: 14, fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit' },
  divider:         { height: 1, background: '#2d2d2d', marginBottom: 20 },
  empty:           { color: '#6b7280', fontSize: 14, padding: '40px 0', textAlign: 'center' },
  songRow:         { display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', borderRadius: 8, transition: 'background 0.15s' },
  rowNum:          { color: '#6b7280', fontSize: 12, width: 20, textAlign: 'center', flexShrink: 0 },
  songCover:       { width: 40, height: 40, borderRadius: 6, objectFit: 'cover', flexShrink: 0, cursor: 'pointer' },
  ...
  songArtist:      { color: '#6b7280', fontSize: 11, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
  songGenre:       { background: 'rgba(34,197,94,0.1)', color: '#22c55e', border: '1px solid rgba(34,197,94,0.2)', borderRadius: 4, padding: '2px 8px', fontSize: 11, fontWeight: 500, flexShrink: 0 },
  reorderBtns:     { display: 'flex', flexDirection: 'column', gap: 2, flexShrink: 0 },
  arrowBtn:        { background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', fontSize: 12, padding: '1px 4px', lineHeight: 1, fontFamily: 'inherit' },
  removeBtn:       { background: 'none', border: 'none', color: '#4b5563', cursor: 'pointer', fontSize: 14, padding: '4px 6px', flexShrink: 0, fontFamily: 'inherit' },
};
