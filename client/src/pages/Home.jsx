 ...
 *
 * BUG 1 FIX: Sentinel div moved INSIDE styles.page and given paddingBottom
 *   that accounts for the fixed player bar height (88px). Previously the
 *   sentinel was outside the page div — the IntersectionObserver could not
 *   see it entering the viewport reliably, so fetchMore() never triggered.
 *
 * BUG 2 FIX: IntersectionObserver now guards with hasNextPage &&
 *   !isFetchingNextPage before calling fetchNextPage(). Previously it called
 *   fetchMore() unconditionally — firing duplicate requests while one was
 *   already in-flight, and calling it endlessly even after all songs loaded.
 *
 * BUG 3 FIX: Removed duplicate flatMap. useSongs() already returns a flat
 *   `songs` array. Home.jsx now uses that directly instead of re-computing
 *   data?.pages?.flatMap() which could throw if any page shape was malformed.
 *
 * BUG 4 FIX: SongList doesn't accept or render loadMore/loadingMore props.
 *   The loading spinner and sentinel are now owned by Home.jsx, not SongList.
 *   SongList is a pure display component — it only receives songs[].
 *
 * BUG 5 FIX: handlePlaySong now uses queueStore.setQueue(queue, startIndex)
 *   instead of playSong(song, queue). After the playerStore refactor, playSong
 *   only accepts a single song. Queue must be set through queueStore.
 */
...
const Home = () => {
  // BUG 3 FIX: Use songs directly from useSongs — no duplicate flatMap
  const {
    songs,
    isLoading: loading,
  ...
  // BUG 1 FIX: sentinel ref — used inside the page container now
  const sentinelRef = useRef(null);

  const { recentlyPlayed, currentSong, isPlaying } = usePlayerStore();
  // BUG 5 FIX: get setQueue from queueStore
  ...
  /* ── BUG 1 + BUG 2 FIX: Infinite scroll observer ────────────────────────
     - Sentinel is now inside the page div (see JSX below)
     - Guard: only call fetchMore when hasMore is true AND not already fetching
     - rootMargin 300px fires before user actually hits the bottom
  ─────────────────────────────────────────────────────────────────────────── */
  useEffect(() => {
  ...
  // Re-attach when hasMore or loadingMore changes so the guard stays fresh
  ...
  // BUG 5 FIX: use setQueue(queue, startIndex) instead of playSong(song, queue)
  const handlePlaySong = useCallback((song, queue) => {
    ...
    return (
      <div style={styles.page}>
        ...
        <div style={styles.errorWrap}>
          ...
          <p style={styles.errorSub}>{error?.message || "Something went wrong. Please try again."}</p>
  ...
  return (
    // BUG 1 FIX: paddingBottom accounts for fixed player bar (72px) + sentinel space
    <div style={{ ...styles.page, paddingBottom: 120 }}>
      ...
      <div style={styles.container}>
            ...
            {showHistory && (
              <div style={styles.historyDropdown}>
                <div style={styles.historyHeader}>
                  ...
                  <button onMouseDown={handleClearAllHistory} style={styles.clearAllBtn}>Clear all</button>
                </div>
                {history.map((item) => (
                  <div key={item.id} style={styles.historyRow} onMouseDown={() => handlePlayFromHistory(item)}>
              ...
              {featuredSongs.map((song) => {
                ...
                return (
                  <div
                    ...
                    style={{ ...styles.featuredCard, outline: active ? "2px solid #22c55e" : "2px solid transparent" }}
                  >
                    ...
                    <div style={styles.featuredInfo}>
                      <p style={{ ...styles.featuredTitle, color: active ? "#22c55e" : "#fff" }}>{song.title}</p>
        ...
        {/* ── BUG 4 FIX: SongList is purely a display component ──────────────
            loadMore / hasMore / loadingMore props removed — SongList doesn't
            use them. The sentinel and spinner below own infinite scroll UI.
        ─────────────────────────────────────────────────────────────────────── */}
        <SongList songs={filtered} />

        {/* ── BUG 1 FIX: Sentinel + spinner INSIDE the page container ────────
            Placing it here ensures the IntersectionObserver can detect it
            entering the viewport. The 300px rootMargin in the observer means
            fetchMore() fires before the user reaches the very bottom.
        ─────────────────────────────────────────────────────────────────────── */}
        {!searchText.trim() && (
          <div ref={sentinelRef} style={styles.sentinel}>
            ...
            {!hasMore && songs.length > 0 && !loadingMore && (
              <p style={styles.allLoadedText}>All {songs.length} songs loaded</p>
            )}
          </div>
        )}
      </div>
...
/* ── Icons ───────────────────────────────────────────────────────────────── */
const StarIcon = () => (
  <svg width="13" height="13" viewBox="0 0 24 24" fill="currentColor" stroke="currentColor" strokeWidth="1" style={{ marginRight: 6, color: "#f59e0b" }}>
...
const PlayIconSmall = () => (
  <svg width="16" height="16" viewBox="0 0 24 24" fill="#000"><path d="M8 5.14v14l11-7-11-7z" /></svg>
);
const PauseIcon = () => (
...
const styles = {
  ...
  historyDropdown: {
    ...
    background: "#1a1a1a", border: "1px solid #22c55e", borderTop: "1px solid #2d2d2d",
    borderRadius: "0 0 12px 12px", overflow: "hidden", boxShadow: "0 8px 32px rgba(0,0,0,0.5)",
  },
  historyHeader: {
    display: "flex", alignItems: "center", justifyContent: "space-between",
    padding: "10px 14px 8px", borderBottom: "1px solid #2d2d2d",
  },
  ...
  historyRow:    { display: "flex", alignItems: "center", gap: 10, padding: "8px 14px", cursor: "pointer", transition: "background 0.1s", borderBottom: "1px solid #1f1f1f" },
  ...
  historyRemoveBtn: { background: "none", border: "none", color: "#4b5563", cursor: "pointer", padding: 4, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, borderRadius: "50%", transition: "color 0.15s" },
  ...
  // BUG 1 FIX: sentinel inside page with enough height to be detectable
  sentinel: { height: 60, display: "flex", alignItems: "center", justifyContent: "center", marginTop: 24 },
  loadingMore: { display: "flex", alignItems: "center", gap: 10 },
  spinner: {
    width: 18, height: 18, borderRadius: "50%",
    border: "2px solid #2d2d2d", borderTopColor: "#22c55e",
...
if (typeof document !== "undefined" && !document.getElementById("home-spin-style")) {
